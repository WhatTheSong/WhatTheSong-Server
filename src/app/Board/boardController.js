...
exports.getRecommendations = async function (req, res) {
  ...
  const recommendationListResult =
    await boardProvider.retrieveRecommendationList(userIdx, writerIdx);
...
exports.getRecommendation = async function (req, res) {
  ...
  const recommendationResult = await boardProvider.retrieveRecommendation(
    recommendationIdx
  );
...
/**
 * API NAME: 노래 질문 게시판 조회 API
 */
exports.getQuestionPosts = async function (req, res) {
  /**
   * 1: 노래 추천 게시판
   * 2: 노래 질문 게시판
   */
