...
exports.getOneComment = async function (postIdx, commentIdx) {
  const connection = await pool.getConnection(async (conn) => conn);
  const commentResult = await commentDao.selectOneComment(
    connection,
    postIdx,
    commentIdx
  );
...
exports.getReplies = async function (postIdx, parentIdx) {
  const connection = await pool.getConnection(async (conn) => conn);
  const replyListResult = await commentDao.selectReply(
    connection,
    postIdx,
    parentIdx
  );
...
exports.getOneReply = async function (postIdx, parentIdx, replyIdx) {
  const connection = await pool.getConnection(async (conn) => conn);
  const replyResult = await commentDao.selectOneReply(
    connection,
    postIdx,
    parentIdx,
    replyIdx
